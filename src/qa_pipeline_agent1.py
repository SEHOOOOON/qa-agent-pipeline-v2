        ...
        *,
        ...
        try:
            response = self.client.responses.parse(
                ...
                store=False,
                prompt_cache_key="qa-v2-agent1-2-14",
                input=[
        ...
        if target:
            for condition in analysis.confirmed_conditions:
                if (condition.source_type == ConditionSource.SRS
                        ...
                        and _normalize(condition.statement) == _normalize(condition.source_text)
                        and _normalize(condition.source_text) in {
                            _normalize(target.statement), _normalize(target.acceptance_criteria)}):
                    delivered_ranges |= _temperature_ranges(condition.source_text) & background_ranges
