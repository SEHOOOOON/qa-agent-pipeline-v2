...
9월 12일 내부 검수 후 최소 보완: 승인 후보 해시는 Agent 3·변경 검증 기록 모두와 일치해야 하며 다르면 승인·승인 전 재검증을 차단합니다. 서로 다른 코드 중 하나를 임의로 공식 등록하지 않습니다. 새 승인 TC 카탈로그에는 원본 TC JSON을 함께 보관하고 보고에서 원본 해시로 확인합니다. 현재 화면 재검증을 거친 공식 자산은 공개 가능한 요약을 `approved_assets/provenance/`에 보존하고 Registry가 그 파일과 원본 로컬 기록의 SHA-256을 각각 기록합니다. 과거 Run의 화면 조회는 지정 자산 폴더를 사용하며 기록을 소급 수정하지 않습니다. Run 전환 시 승인 확인·SRS 동의·판단 메모 초기화, GET 10초/POST 180초 대기, Agent 4 설명 영역 겹침을 보완했습니다. 문자열 포함·해시 보조 함수와 CP4 중복 비교만 정리했고 QA 기준·과거 호환·코드 재생성 비교는 유지했습니다. 새 모델 API·외부 전송·공식 등록은 수행하지 않았습니다.

9월 11일 UI 연결 최소 보완: 실행 중 재접속·창 닫기 후 갱신 유지, 통신 장애 표시·재시도, 조회 선택 유지와 늦은 조회 응답 무시를 적용했습니다. 로컬 실행기는 명시 Run ID와 설정 SRS·공식 자산 경로를 전달하며 다른 실행 폴더를 추측해 선택하지 않습니다. CLI `pipeline --run-id`는 선택 사항이고 기존 ID 재사용은 거부합니다. 테스트는 별도 브라우저에서 수행한다는 안내와 파일 데모/HTTP 장애 구분을 추가했습니다. QA 판정·제품 제어 동작·공식 자산·과거 Run은 변경하지 않았으며, HTML 해시 변경 때문에 과거 후보 승인 전 현행 제품 재검증은 필요합니다. 새 API·외부 전송·공식 승인은 수행하지 않았습니다.
...
| 중앙제어 Run 화면 | 공개 접속은 MED 풍량 정상 변경 V2 데모, 로컬 서버는 저장 결과 조회·API 새 실행·후보 공식 자산 판단을 분리. 데모 승인에는 실제 파일 반영이 없고 로컬의 새 API 실행과 사람 자산 승인은 각각 기본 잠금, Agent 4 외부 보고는 미리보기 고정 |
...
- 9월 12일 공개 정상 변경 데모 보완 후 전체 207건 통과, 수집 207건 확인. 공개 데모의 API 호출 0회·승인 후 실제 파일 미변경과 로컬 Live 연결 모드 보존을 검사했으며 새로운 실제 API 성공 기록은 추가하지 않았습니다.

- 9월 11일 최소 UI 연결 수정 후 관련 50건 통과, 전체 203건 통과(79.77초), 수집 203건·diff 검사 확인. 신규 브라우저 테스트는 서버 응답을 대체한 장애·복구·선택 검증이며 실제 API 전체 실행은 아님.
