...
현재 자동 테스트는 **201건**이며 9월 9일 전체 통과(87.17초), 9월 10일 커밋 전 전체 통과와 수집 201건을 확인했습니다. 실제 브라우저의 지연 반영과 로컬 HTTP를 통한 임시 SRS 승인도 포함합니다. 개별 TC 통과와 미확인 범위를 분리하고, 자동화 제외·정보 부족이 남으면 최종 권고를 사람 검토로 표시합니다. 실행은 계속하며 과거 Run 결과는 덮어쓰지 않습니다. 화면·사람 검토서·외부 보고 미리보기에도 검증 범위를 표시합니다.
