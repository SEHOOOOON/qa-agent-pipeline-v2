...
기존 TC를 선택한 것과 특정 조건의 검증 근거로 연결한 것은 구분합니다. 복합 조건을 여러 TC가 나누어 확인하면 각각에 같은 조건 ID를 연결하도록 안내합니다. Agent 2 입력에는 조건별 코드·수치와 재작성 시 현재 연결·미충족 값을 함께 제공합니다. 제한된 진단 자료이며 자동 연결·의미 합격 판정은 아닙니다. 실제로 검증하지 않는 값이나 반대 동작은 기존 검사에서 계속 차단합니다.
