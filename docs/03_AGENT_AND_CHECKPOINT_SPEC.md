...
기존 TC 전용 경로: 신규 후보가 없고 `관련_기존_TC`를 선택했으면 CP2-014는 새 후보의 준비·복원 필드를 요구하지 않습니다. 대신 실행기는 해시를 확인한 요청의 시험 절차 메모와 기존 TC ID를 `최종_확인_사항`에 결정론적으로 기록하고 Agent 4·사람 검토서까지 전달합니다. 기존 코드가 해당 절차를 수행했다고 추정하거나 메모를 제외 범위로 버리지 않습니다. 후보가 있는 요청의 절차 보존 검사와 기존 TC 선택·조건 연결 검사는 유지합니다. 신규 Agent 2 Manifest의 `existing_procedure_review_contract=1.0`으로 이 경로를 구분하고, 해당 표시가 없는 역사적 Run은 당시 CP2 규칙으로 재계산합니다. 기존 자동화의 준비·복원 적합성 자체를 자동 증명하는 기능은 아닙니다.
...
Trial의 원본 실행 상태와 최종 원인 분류는 구분합니다. 제품 Assertion 실패와 복원 불일치가 함께 있으면 Trial은 제품 불일치 후보로 기록될 수 있지만, Agent 4는 SHA-256이 일치하는 stdout·stderr의 실제 `RESTORE_MISMATCH` 행을 확인해 자동화 실행 문제·HOLD로 분류합니다. 사람 검토서에는 제품 불일치 관찰과 복원 실패를 함께 표시하며, 같은 TC의 실행 수나 검토 항목을 중복 집계하지 않습니다. 소스 코드에 인용된 표식이나 해시가 다른 로그를 복원 실패 근거로 쓰지 않습니다. 증거 불일치는 기존 CP4 차단 규칙을 따르며, 이 보완이 모든 복원 실패의 검출을 보장하지는 않습니다.
...
기존 TC 전용 Run은 같은 승인 화면의 SRS 문서 승인과 asset-decision API의 SRS_ONLY 대상으로 문구만 승인할 수 있습니다. Agent 1/2 인계, 보고·실행 해시, 선택 기존 TC의 통과·증거·현재 제품 해시, 제안의 기존 TC Condition 연결을 확인합니다. 검토자·필수 판단 메모·SRS 반영 동의와 두 번의 화면 확인을 요구합니다. 현재 SRS 원문이 충돌하면 거부하며 승인 기록은 Run의 srs_only_decision.json과 approved_assets/srs_revisions/<Run-ID>.json에 보존합니다. TC·자동화·Registry는 변경하지 않고 기록 실패 시 SRS를 원상복구합니다.

9월 9일부터 신규 최종 보고는 실행한 후보 유무와 무관하게 자동화 제외 또는 정보 부족이 남으면 PASS 대신 HUMAN_REVIEW를 권고합니다. 개별 실행 결과·CP4와 실행 계속 정책은 유지하며 제외를 제품 결함으로 집계하지 않습니다. 의도적으로 범위 밖으로 정한 항목만으로는 실패 처리하지 않습니다. 화면·사람 검토서·외부 보고의 검증 범위 설명은 기록된 실행 상태를 설명하며 자연어 요구사항 전체의 의미적 충족을 보장하지 않습니다.

## 8. Run 요약

단일 agent3 CLI는 루트 agent3_manifest.json·agent3_trial.json을 쓰며 실행 요약은 만들지 않습니다. CP4는 정확한 루트 경로·원본 해시·후보 코드·증거를 확인해 이 인계를 허용합니다. 다중 실행은 계속 요약 파일과 해시를 요구합니다.

Agent 2 agent2-2.20부터 관찰 ER 분리와 조작 반복을 구분합니다. 준비·복원 전용 값은 시험 요청값에 섞지 않습니다. 신규 double_assert_timing_contract=1.0의 CP2-006은 REQUIRED 정책의 UI·내부 결과가 같은 판정 단계 집합을 사용하는지 검사합니다. 단일 흐름의 두 계층 모두 판정 단계 미지정이면 기존 종료 시점 검증을 유지합니다. 자연어 의미의 완전한 일치를 증명하는 검사는 아니며 과거 CP2에는 소급 적용하지 않습니다.
