...
HTML 파일을 직접 열거나 공개 웹 주소에서 접속하면 MED 풍량 정상 변경 1건의 저장된 V2
데모를 같은 패널에 표시합니다. 이 모드는 Agent 1~4 단계·TC 상세·사람 승인 화면을
설명하기 위한 것으로 API 호출, 제품 자동화, 외부 전송, SRS·공식 TC 파일 반영을 하지
않습니다. `localhost`·`127.0.0.1`의 로컬 브리지 모드와 구분합니다.
...
현재 화면 재검증 뒤 승인한 경우 Raw Screenshot·Trace는 Git 제외 Run에 보존하고, 공개
자산에는 경로·비밀정보를 제외한 재검증 요약을 둡니다. Registry는 이 요약 파일 해시와
원본 로컬 재검증 기록 해시를 구분해 기록합니다.
