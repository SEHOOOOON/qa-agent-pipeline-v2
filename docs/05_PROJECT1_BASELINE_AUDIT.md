...
2~8절의 V1 기능·Coverage·표현 감사는 위 기준일의 역사적 기록입니다. `REQ-LOCAL-*` 및 중앙·현장 조작은 당시 V1 자산 설명이며 V2 지원 범위가 아닙니다. V2의 현재 상태는 [인계 문서](../PROJECT_HANDOFF.md)와 아래 V2 보완 이력을 기준으로 읽습니다.
...
### 2026-09-06 TC 화면·분류 비교

사용자 캡처와 로컬 `portfolio_export/project.html`, `scripts/agent4_reporting.py`를 대조했습니다. 온라인 페이지의 현재 배포본까지 동일하다고 확인한 것은 아닙니다.

| 항목 | V1 기준 | V2 현재 |
|---|---|---|
| TC 설계 유형 | 해피패스·엣지케이스·예외/결함·사전 점검 | 구조화 TC 유형을 표시하고 상태 정합성도 구분 |
| 실행 후 분류 | 규칙 기반 요구사항 검토·환경·자동화·미실행·제품 결함 분류 | 규칙 기반 제품 불일치 후보·자동화·환경·근거 부족·미실행 분류 유지 |
| TC 표 | 미리 구성한 행과 상세 내용 | 선택 Run의 실제 설계·실행·제외·보고 기록을 결합하고 유형 필터·행 펼침 제공 |
| 상세·우선순위 | 페이지/코드에 미리 지정 | 신규·승인 TC의 기록된 명세 표시, 기록 없는 기존 상세·유형 및 우선순위는 추정하지 않음 |
| 외부 보고 | Slack·Notion, TC ID 기준 갱신 | CP4 통과 뒤 명시적 전송, Notion은 Run ID+TC ID로 실행 간 보존 |

V2의 Agent 4 분류 기능이 빠졌던 것은 아닙니다. 이번 보완은 분류 체계를 확대하기보다 사용자가 개별 TC·실행 상태·원인을 함께 확인할 수 있게 연결한 것입니다. 신규 실제 Live 및 외부 전송은 이번 화면 검증에 포함하지 않았습니다.
...
적용됐고 Project1 원본과 공식 TC 자산은 변경하지 않았습니다. 2026-09-12에는 공개
접속에서 같은 V2 패널로 MED 정상 변경 저장 데모를 보여주도록 보완했으며, 이 데모는
API·제품 자동화·SRS·공식 TC 파일을 변경하지 않습니다.
