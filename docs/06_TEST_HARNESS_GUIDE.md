...
로컬 UI는 테스트 브라우저의 실시간 중계가 아니라 실행 요청·결과 조회 화면입니다. 상태 조회는 10초, 승인·후보 재검증 POST는 180초 응답 대기 제한을 사용합니다. 후보 시험 자체의 기본 제한은 90초로 유지하며, HTTP 대기 종료가 서버 시험 취소를 의미하지는 않습니다. Run이 바뀌면 사람 승인 확인·SRS 문구 동의는 다시 받아야 합니다.

9월 9일 결과 반영 대기: 신규 컴파일러는 고정 100ms 뒤 한 번 읽는 대신 같은 판정 단계의 관찰 묶음을 최대 2초 동안 50ms 간격으로 다시 읽습니다. 조작·입력·적용은 반복하지 않습니다. 해당 회차의 UI·내부 기대 결과가 모두 맞으면 진행하며 끝까지 다르면 기존 불일치 분류를 유지합니다. 복원 결과도 같은 방식으로 확인합니다. 이 값은 제품의 성능 요구사항이 아니라 현재 실행기의 기술적 대기 한도입니다. 이미 기대값과 같은 미변경 조건은 즉시 일치할 수 있어, 실제 장비의 응답 완료나 일정 시간 동안 불변이라는 성질을 이 대기만으로 증명하지 않습니다.
...
PASS와 PRODUCT_MISMATCH_CANDIDATE는 필수 증거와 해시 검사 뒤 다음 단계로 인계합니다. 다만 현재 `evidence_complete`는 Screenshot·Trace 파일 존재 여부이며 복원 성공까지 뜻하지 않습니다. 제품 불일치와 복원 실패가 동시에 발생하면 Trial 원본 상태와 별도로 Agent 4가 해시가 일치하는 실제 복원 실패 로그를 확인해 자동화 실행 문제·HOLD로 판단하고 두 관찰을 함께 보고합니다. 표식이 기록되지 않은 실패까지 자동 검출한다는 뜻은 아닙니다. 세부 조건은 [Agent·Checkpoint 명세](03_AGENT_AND_CHECKPOINT_SPEC.md)에 기록합니다.
