...
아래는 완료를 검토할 때 적용하는 체크리스트이며 모든 항목의 현재 달성을 선언하는 목록은 아닙니다. 실제 통과·실패 이력은 아래 날짜별 기록, 기존 TC 전용 처리와 복원 분류의 미해결 제한은 [Agent·Checkpoint 명세](03_AGENT_AND_CHECKPOINT_SPEC.md)를 함께 확인합니다.
