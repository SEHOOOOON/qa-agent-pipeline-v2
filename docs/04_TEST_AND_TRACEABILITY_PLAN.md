...
- CP3는 Selector-행동 대응, Expected Result 1:1, 관찰 계층, 값 근거, 장비 복합 내부 검사의 관찰 필드명, 단계 순서와 초기값 복원 Action을 검사합니다. 컴파일러는 복원 후 기존 온도뿐 아니라 범용 UI·내부 상태도 초기 관찰값과 비교합니다.
...
- `agent4` CLI는 실행 결과를 다시 실행하거나 모델 API를 호출하지 않고 `agent4_analysis.json`·`checkpoint4.json`·`final_report.json`을 생성합니다.
