...
| Agent 2 | V1·승인 공식 TC Requirement·검증 동작 대조, Agent 1 VERIFY 회귀의 결정론적 보충, 변경분 후보·관련 기존 TC 분리, 확정 Condition 처리 경로, 업무 규칙 단위 조건 묶음, 조건별 판정·초기화, SRS 개정 제안, 3단계 QA 기준, 추적성, 독립성, 이중 검증 |
...
| 변경 검증 | 현재 CP3 재검사, 후보 재사용/재시험, 환경 점검, Agent 2가 선택한 V1·승인 공식 TC 실행과 자산 해시 확인 |
